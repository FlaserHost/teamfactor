...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        calculateBtn.addEventListener('click', () => {
            const calculateData = [...new FormData(calculationForm)];
            let number_of_employees = +calculateData[0][1];
            let internal_customers = +(calculateData[1][1]).split(' ')[0];

            const leftMonth = number_of_employees * middle / 12;
            const rightMonth = priceList[internal_customers];
